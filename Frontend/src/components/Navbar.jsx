...
export default function Navbar() {
  ...
  return (
    <nav className={`sticky top-0 z-50 transition-all duration-300 ${
      scrolled
        ? 'bg-white/90 backdrop-blur-lg shadow-md border-b border-gray-100/80'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3 min-w-0">
            {user ? (
              <>
                ...
                {user?.role !== 'admin' && (
                  <Link to="/notifications"
                    className={`relative flex-shrink-0 p-2 rounded-xl transition-all duration-200 ${
                      isActive('/notifications') ? 'bg-[#FFF0F0] text-[#C0162C]' : 'text-[#1A1A2E]/60 hover:bg-gray-100 hover:text-[#1A1A2E]'
                    }`}
                    title="Notifications">
                ...
                <div className="flex items-center gap-2 pl-3 border-l border-gray-200 min-w-0">
              ...
              <>
                <Link to="/login" className={navLinkCls('/login')}>Login</Link>
          ...
          {/* Mobile hamburger */}
          <button
            className="md:hidden p-2 rounded-xl text-[#1A1A2E]/70 hover:bg-gray-100 hover:text-[#C0162C] transition-colors"
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white/95 backdrop-blur-lg px-4 py-4 space-y-2"
          style={{ animation: 'slideUp 0.2s ease-out' }}>
          {user ? (
            <>
              <div className="flex items-center gap-3 p-3 rounded-xl bg-[#FFF0F0] mb-2">
              ...
              <Link to={dashboardPath} onClick={() => setMenuOpen(false)}
                className="flex items-center gap-2 py-2.5 px-3 rounded-xl text-[#1A1A2E] hover:bg-[#FFF0F0] hover:text-[#C0162C] font-medium transition-colors">
                📊 Dashboard
              </Link>
              {user?.role !== 'admin' && (
            ...
            <>
              <Link to="/login" onClick={() => setMenuOpen(false)}
                className="flex items-center gap-2 py-2.5 px-3 rounded-xl text-[#1A1A2E] hover:bg-[#FFF0F0] font-medium transition-colors">
                Login
              </Link>
